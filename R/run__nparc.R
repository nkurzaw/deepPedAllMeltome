...
proteoforms_ratios_wo_rep <- 
    proteoforms_ratios[,-grep("_BR2", pData(proteoforms_ratios)$sample_name_machine)]

# compute null model
nparc_null_res_df <- fit_and_eval_melting_curves(
    proteins = proteoforms_ratios_wo_rep, 
    sample_meta = sample_meta_raw,
    by_var = "none",
...
saveRDS(nparc_null_res_df, file = file.path(output_folder, "nparc_res_hq_only_null_model.RDS"))
