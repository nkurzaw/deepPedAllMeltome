...
export_to_viz <- function (iois,
    ...
    write_csv(x = alignment,
              file = file.path(output_folder, "alignment.csv"),
              col_names = TRUE,
              escape = 'none')
    # write_csv(x = nodes,
    #           path = file.path(output_folder, "nodes.csv"),
    #           col_names = TRUE,
    #           quote_escape = FALSE)
    # 
    # write_csv(x = edges,
    #           path = file.path(output_folder, "edges.csv"),
    #           col_names = TRUE,
    #           quote_escape = FALSE)
    # 
    # write_csv(x = alignment,
...
peptide_features_df <- tibble(
    ...
    gene_symbols = featureData(peptides)$id,
